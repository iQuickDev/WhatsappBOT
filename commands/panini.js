	...
	execute: async (message, info) => {
		...
		for (const sandwichorder of sandwiches.orders) {
			for (const participant of chat.participants) {
				if (sandwichorder.customer.id.user == participant.id.user) {
					const contact = await index.client.getContactById(
		...
		for (const sandwichorder of sandwiches.orders) {
			...
			for (let i = 0; i < sandwichorder.name.length; i++) {
				...
				else sandwichesString += ` or ${sandwichorder.name[i].toUpperCase()}`
			}

			sandwichesString += `\nPaid: ${
				sandwichorder.paid
			} € | Change: ${sandwichorder.change.toFixed(2)} €\n\n`

			total += sandwiches.list.find(
				(x) => x.name == sandwichorder.name[0]
			).price
