...
module.exports = {
	...
	execute: async (message, info) => {
		if (info.args[0] == '') {
			message.reply(
				`*Syntax*: wp panino <name>,<alternative> <paid money> (alternatives are unlimited)\n*Example*: wp panino crf,tonno 3`
			)
			return
		}
		...
		try {
			change =
				givenMoney -
				sandwiches.list.find((x) => x.name == sandwichNames[0]).price
		} catch (error) {
			message.reply(
				'*REJECTED*: Please specify a valid panino and amount of money'
			)
			return
		}
		...
		for (let i = 0; i < sandwichNames.length - 1; i++) {
			if (
				sandwiches.list.find((x) => x.name == sandwichNames[i]).price !=
				sandwiches.list.find((x) => x.name == sandwichNames[i + 1]).price
			) {
